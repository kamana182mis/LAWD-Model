...
#        ylab         = "Deviance Residuals", grid = F , id = F)
...
LogL2 <- function(x) {
  ...
  for (j in 1:length(data)) {
    ...
    # Ensure alp is positive
    if (alp < 0) {
      alp <- 1e-6  # Small positive value
    }
    
    term1 <- alp * the * (data[j]^(the - 1)) + bet * gam * (data[j]^(gam - 1))
    term2 <- alp * (data[j]^the) + bet * (data[j]^gam)
    term3 <- 1 + (term2)^et
    
    # Prevent log(negative)
    term1 <- max(term1, 1e-10)
    term2 <- max(term2, 1e-10)
    term3 <- max(term3, 1e-10)
  ...
  for (k in 1:length(dc)) {
    ...
    term4 <- 1 + (alp * (dc[k]^the) + bet * (dc[k]^gam))^et
    
    term4 <- max(term4, 1e-10)
...
optim(
  par = rep(0.001,5),
  fn = LogL2,
  lower = c(0.001, 0.001, 0.001, -Inf, -Inf)/1000,
  upper = rep(Inf, 5),
  method = "L-BFGS-B", hessian = T
)
...
2*length(res2$par)-(-2*res2$value)
log(60)*length(res2$par)-(-2*res2$value)
...
# Run optimization
res1 = optim(
  par = c(0.00001000, 0.73163421, 0.01200455, 0.11642635)*2,
  fn = LogL1,
  lower = c(0.001, 0.001, -Inf, -Inf)/100,
  upper = rep(Inf, 4),
  method = "L-BFGS-B"
)

2*length(res1$par)-(-2*res1$value)

log(60)*length(res1$par)-(-2*res1$value)
